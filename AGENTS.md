...
- **内置查询**（`extensions/ts-search/`）：独立的网络模块，为支持内置查询的模型（GPT / Grok）追加 `web_search` 工具。
...
- 结构：`extensions/index.ts`（薄入口）组装两个平行模块——`models/`（模型目录 + 思维链调度）与 `ts-search/`（内置查询），两模块互不依赖。
- 测试：4 个测试文件（index / models/operations / models/catalog / ts-search），纯 npm 生态（tsc 编译 + node 运行），`FakePi` / `FakeContext` 模拟宿主，`PI_CODING_AGENT_DIR` 隔离配置目录。`npm run test` 全绿。
...
| 测试 | `npm run test` = `tsc && node dist/extensions/index.test.js && node dist/extensions/models/operations.test.js && node dist/extensions/models/catalog.test.js && node dist/extensions/ts-search/ts-search.test.js` |
...
extensions/ts-search/         # 网络模块（平行、独立）：内置查询工具注入
├── ts-search.ts              # BUILTIN_SEARCH_MODELS 名单 + applyWebSearchTool
└── ts-search.test.ts
```

## 关键约束（改动前必读）

1. **宿主耦合只允许在 `extensions/index.ts`**。`models/`、`ts-search/` 保持纯逻辑，保证可在无 Pi 环境直接测试。`index.ts` 是唯一的组装点（两模块互不感知）。
...
Only declared root fields, the three declared Google `config.thinkingConfig` fields, and the dedicated ts-search append can change. There is no generic JSON Patch, history/messages rewrite, response parser, auth/header/URL alteration, cache-key alteration, or token-limit alteration.
...
2. **Grok 支持**：查证 xAI 内置查询工具格式后加入 ts-search 模块名单；按需把 Grok 模型加入目录。
