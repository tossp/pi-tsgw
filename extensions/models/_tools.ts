...
export function applyEnabledThinking(writer: PayloadWriter, level: ThinkingLevel): void {
	writer.remove("reasoning_effort");
	writer.setThinking({ type: level === "off" ? "disabled" : "enabled" });
}

/** 厂商思维链策略：对指定模型的请求体应用该厂商的 thinking 改写。 */
export type ThinkingApplier = (writer: PayloadWriter, context: ModelOperationContext) => void;